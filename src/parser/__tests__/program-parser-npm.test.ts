...
const npmParser = new ProgramParser({
  ...
  childPrograms: [
    new ProgramParser({
      ...
      primaryArguments: [],
      optionalArguments: [],
      additionalArguments: { name: 'packages', type: new TypeStringSchema({}) },
      options: [
    ...
    new ProgramParser({
      ...
      optionalArguments: [],
      additionalArguments: { name: 'args', type: new TypeStringSchema({}) },
      options: [
...
describe('ProgramParser npm schema', () => {
  it('parses install with mixed aliases and workspaces', async () => {
    ...
    expect(result.result.additionalArguments).toEqual(['react', 'typescript'])
  })
